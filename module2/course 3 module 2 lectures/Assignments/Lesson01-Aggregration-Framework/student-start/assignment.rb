...
class Solution
  ...
  def concat_names
    #place solution here
    @coll.aggregate(
      [{:$project => {:_id => 0, :number => 1, :name => {:$concat => ['$first_name', ' ', '$last_name']}}}]
    )
  end
  ...
  def group_last_names
    #place solution here
    @coll.aggregate(
      [:$group => {:_id => {:gender => '$gender', :age => '$group'}, :last_names => {:$push => '$last_name'}}]
    )
  end
  ...
  def group_last_names_set
    #place solution here
    @coll.aggregate(
      [:$group => {:_id => {:gender => '$gender', :age => '$group'}, :last_names => {:$addToSet => '$last_name'}}]
    )
  end
  ...
  def age_groups_faster_than age_group, criteria_time
    ...
    @coll.aggregate([
      ...
      {:$group => {:gender => '$gender'}, :runners => {:$sum => 1}, :fastest_time => {:$min => '$secs'}},
